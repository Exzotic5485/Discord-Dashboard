    ...
    props.fastify.register((instance: any, opts: any, next: any)=>{
        instance.get('/callback', async (request: any, reply: any) => {
            if(request.query?.guild_id){
                if(!props.discordClient.guilds.cache.get(request.query.guild_id))
                    await props.discordClient.guilds.fetch(request.query.guild_id)
                if(!props.discordClient.guilds.cache.get(request.query.guild_id).members.cache.get(request.session.user?.id))
                    await props.discordClient.guilds.cache.get(request.query.guild_id).members.fetch(request.session.user?.id)
